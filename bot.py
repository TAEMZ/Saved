...
async def list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = update.effective_chat.id
    loading_msg = await update.message.reply_text("⏳ Loading your saved messages...")
...
async def view_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = update.effective_chat.id
    match = re.match(r"^/view(\d+)(?:@\w+)?$", update.message.text)
    if not match:
        return
...
async def archive_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    match = re.match(r"^/archive(\d+)(?:@\w+)?$", update.message.text)
    if not match:
        return
    db_id = int(match.group(1))
